...
def _extract_a2ui_messages(text: str) -> list[dict]:
    ...
    for value in values:
        ...
        if "components" in value and isinstance(value.get("components"), list) and len(value["components"]) > 0:
            surface_id = value.get("surfaceId", "default")
            root_id = value["components"][0].get("id", "root")
        ...
        elif "surfaceUpdate" in value and isinstance(value.get("surfaceUpdate"), dict):
            su = value["surfaceUpdate"]
            comps = su.get("components", [])
            surface_id = su.get("surfaceId", "default")
            if comps and not any("beginRendering" in m for m in messages):
                root_id = comps[0].get("id", "root")
                messages.append({"beginRendering": {"surfaceId": surface_id, "root": root_id}})
            messages.append(value)
        elif any(k in value for k in ("beginRendering", "dataModelUpdate", "deleteSurface")):
            messages.append(value)
    return messages
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        if "<a2ui-json>" in text:
            before_tag = text.split("<a2ui-json>", 1)[0].strip()
            if before_tag:
                prose_parts.append(types.Part(text=before_tag))

        new_parts = prose_parts + [_wrap_a2ui_part(m) for m in messages]
        return LlmResponse(
